...
def k_means(burst_peaks, burst_borders, sdf, fs=12500):
    ...
    n1_y, bins1_y = np.histogram(cluster_y_1, bins=num_of_bins, range=rng_y)
    n2_y, bins2_y = np.histogram(cluster_y_2, bins=num_of_bins, range=rng_y)
    ...
    if (percent_overlapping_x > 0.20) | (percent_overlapping_y > 0.15):
        print(f"Well is probably not reverberating.")
        return False, percent_overlapping_x, percent_overlapping_y, x, y, kmeans.labels_, kmeans.cluster_centers_, dst
    else:
        print(f"Well is probably reverberating.")
